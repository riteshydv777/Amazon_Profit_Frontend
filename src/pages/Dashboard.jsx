...
export default function Dashboard() {
  ...
  const handlePaymentUpload = async () => {
    ...
    try {
      ...
      if (Array.isArray(skuList) && skuList.length > 0) {
        ...
        const uniqueSkus = Array.from(uniqueSkusSet).sort();
        console.log("🔍 Deduplicated SKUs:", uniqueSkus);
        setSkus(uniqueSkus);
        ...
        console.warn("❌ No SKUs found");
        setMessage("⚠️ No SKUs found. Please ensure files are uploaded correctly.");
      }
      setStep(3);
    } catch (err) {
  ...
  const handleSkuCostSubmit = async () => {
    ...
    try {
      ...
      console.log("📊 Fetching detailed profit report...");
      ...
      console.log("🎯 Setting reportData and moving to step 4...");
  ...
  const downloadReport = () => {
    ...
    const rows = reportData.skuWiseDetails.map((sku) => {
      const profit = sku.totalProfit || 0;
      const revenue = sku.settlement || 0;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {step === 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tools.map((tool) => (
              <div key={tool.id} onClick={tool.action} className="cursor-pointer group">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                    ...
                    <div className="bg-white p-3 rounded-xl border border-blue-100">
                      ...
                      <p className="text-sm font-bold text-slate-700">
                        {formatDate(orderSummary.dateFrom)} <br/> to {formatDate(orderSummary.dateTo)}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {step === 1 && (
                <div className="max-w-2xl mx-auto">
                  <div className="mb-8">
                    ...
                    <p className="text-slate-500">Please provide your Amazon Order file (CSV or TXT)</p>
                  </div>
            ...
            <div className="p-8 md:p-12">
              {reportData ? (
                <DetailedProfitReport
                  ...
                  onStartOver={() => {
                    if (window.confirm("Are you sure you want to start over? Current analysis will be cleared.")) {
                      setStep(0);
                      setOrderFile(null);
